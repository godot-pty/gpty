...
**GDScript coverage:** concept merge/save/load, terminal manager tile lifecycle (spawn, kill, swap, labels, grid-full refusal), divider edge drag (`test_pane_edge_drag.gd`: pixel preview vs unit commit, one `tiles_resized` per release, MIN_TILE clamp, window-border refusal, drag survives leaving the strip), settings save/load roundtrip, profile CRUD and the three profile sources (the shipped built-in, installed-plugin profiles, user profiles — merge order, ` (n)` dedupe, and the plugin-revision consent gate), layout save/restore, sidebar signal emission, pane settings application, IPC dispatch against a real workspace (`test_ipc_dispatch_contract.gd`: listed pane ids and legacy labels both address the pane they name, terminal-only methods refuse other panes, unknown methods answer -32601, an untrusted profile is refused), palette command generation.

**Known gaps (not automatable in headless CI):**
- Real PTY output timing: a headless pane spawns a real shell, but tests that assert on its output are timing-dependent, so they live in the live smoke (`scripts/smoke_pane_api.py`) instead — one harness for every platform, run by `ci-check` and by the `windows-smoke` CI job. Its concept step asserts a routed capture event fired by a runtime-built output marker, which is the live proof that committed *output* lines (ConPTY renders their breaks as cursor moves) reach the concept matcher on the platform running the smoke. Its final step seeds an installed plugin's record into the sandboxed store and asserts the plugin's profile is listed by `layout list` and restored by `layout load`, which is the live proof of the profiles-migration read path.
