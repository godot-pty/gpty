...
- [ ] Trust gate on the IPC restore path — `ipc_handlers.gd` `layoutLoad` calls `ws._do_activate(profile)` directly while the gate lives in `_activate_profile`, so `gpty layout load <name>` and the `layout-load` MCP tool restore an untrusted profile with no consent step. The dialog cannot be answered by a caller: refuse untrusted profiles over IPC with an error pointing at the GUI.
- [ ] Inspector prompt fence — `gpty-ai/src/prompt.rs` wraps untrusted capture in a literal ``` fence with no escaping and interpolates `concept_name`/`source_pane` raw, so a capture containing a fence escapes the data block (omp-only, tool-free backend, so display-level). Use a fence longer than the longest backtick run in the capture and escape the metadata lines.
- [ ] A notify-only match must not shadow a capture — `concept::match_line` returns the first match and `apply_match` starts nothing for `SingleLine`, so a broad notify-only concept placed ahead of a capture concept silently suppresses that capture. Prefer the first capturing match, or act on both.
