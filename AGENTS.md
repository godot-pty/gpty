...
- Concept action routing fails silently when two things are wrong, so check both when actions "do nothing": terminal ids must be unique process-wide (`start_shell` draws from a global counter — the engine's self-reaction guard drops an event when `source_pane == my_id`, and a per-instance counter gives every pane id 1, so every pane discards every event), and `TerminalConfig.labels` must be non-empty (an action's `target` is matched against it, never against anything else). Labels are the pane's `attachment_id` plus its `tags` (`terminal_pane.gd:_concept_labels()`); `pane_label` is deliberately excluded because restore reassigns it. Neither failure raises an error or breaks a test.
