...
- `log::*` goes nowhere in the GUI: no `log` bridge is installed in the GDExtension (the CLI installs `env_logger`), so a `log::warn!` from `gpty-core` is silently dropped — use `godot_warn!`/`godot_error!` for anything a GUI user must see, and do not mistake a missing core warning for a code path that did not run (a probe printed nothing until it was switched to `godot_warn!`). A roadmap item tracks forwarding `warn`+ from the gpty crates to Godot's logger; when that lands, this bullet becomes "core warnings do reach the UI" and must be rewritten with it, not left behind.
