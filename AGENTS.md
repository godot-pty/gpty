...
- Restore vs. user intent: `PaneTypes.tile_spawns_untrusted(td, default_program, default_env)` is the single Workspace Trust predicate — program (`command` > `shell`), non-empty `shell_args`, and a `shell_env` that differs from the user's own settings all count as a new decision. Never add a restore path that spawns from tile data without it (checking only `shell` let an argv/env payload execute with no prompt), and never compare a different key than the spawn path consumes.
