...
const BLOCKED_ENV_KEYS: &[&str] = &[
    ...
    // sanitize_envs() runs, so listing them here is what stops a pane's own
    // environment (the user-owned pane-env store) from re-adding them
    // afterwards and letting a child shell steer the workspace's control
    // socket.
    "GPTY_SECRET",
    "GPTY_SOCKET",
    "GPTY_GUI",
...
///
/// `path_value` is the child's `PATH`: the pane's own environment override
/// (the user-owned pane-env store) when it has one, otherwise the user's
/// global — the same value `execvp` will search,
...
impl PtyHandle {
    ...
    pub fn spawn(
        ...
        // are never accepted from user-authored pane environment data.
...
mod tests {
    ...
    fn sanitize_envs_drops_control_credentials_from_untrusted() {
        ...
        // pane's own environment (the user-owned pane-env store) could re-add
        // GPTY_SOCKET and let the child shell point at an attacker-controlled
        // control socket.
