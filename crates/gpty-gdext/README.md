...
| Method | Returns | Description |
|--------|---------|-------------|
| `start_shell(cmd: String, rows: int, cols: int, envs: String, pane_id: String, history_lines: int)` | void | Start a PTY session (injects per-PTY event capability, `GPTY_ENV=1`, `GPTY_PANE_ID`); attaches the SQLite history store keyed by `pane_id` and restores the newest `history_lines` rows into scrollback |
