//! # godopty — terminal workspace CLI
//!
//! Controls the godopty GUI over JSON-RPC IPC.
//! When the GUI is not running, `godopty` can auto-spawn it
...
use godopty_ipc::transport;

#[derive(Parser)]
#[command(
    name = "godopty",
    version,
    about = "Control the godopty terminal workspace",
    long_about = None
)]
struct Cli {
    #[command(subcommand)]
...
async fn main() {
    ...
    let socket_path = cli.socket.or_else(|| {
        std::env::var("GODOPTY_SOCKET").ok()
    ...
    match &cli.command {
        ...
        Commands::Version => {
            println!("godopty {}", env!("CARGO_PKG_VERSION"));
    ...
    if !cli.no_daemon {
        if let Err(_) = commands::daemon::ensure_running(&socket_path, timeout).await {
            eprintln!("error: could not connect to godopty GUI");
            eprintln!("  Is godopty running? Start it or pass --no-daemon to skip.");
            process::exit(1);
        }
    }
